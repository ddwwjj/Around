...
func main() {
	...
	if !exists {
		// Create a new index.For other fields (user, message, etc.) no need to have mapping as they are default. For geo location (lat, lon), we need to tell ES that they are geo points instead of two float points such that ES will use Geo-indexing for them (K-D tree)
		mapping := `{
                    "mappings":{
                           "post":{
                                  "properties":{
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	//INDEX:around,Query:range+lat+lon
	...
	var typ Post
	var ps []Post
	for _, item := range searchResult.Each(reflect.TypeOf(typ)) { // instance of post
	...
	//convert Go slice to json string
